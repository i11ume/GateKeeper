(function( $ ) {
	...
	$.fn.ghostHunter 	= function( options ) {
	    ...
	    if( opts.results ) pluginMethods.init( this , opts.rss , opts.results , opts.template );
	...
	$.fn.ghostHunter.defaults = {
		results 		: false,
		rss 			: "/rss",
		template 		: "<a href='{{link}}'><p><h2>{{title}}</h2><h4>{{pubDate}}</h4></p></a>"
	};
	...
	var pluginMethods 	= {
		...
		init 			: function( target , rss , results , template ){
		...
		find 		 	: function( value ){
			...
			for (var i = 0; i < searchResult.length; i++)
			{
				...
				results.append(this.format(this.template,postData));
			}
		},
